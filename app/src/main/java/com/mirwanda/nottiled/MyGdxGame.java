...
//////////////////////////////////////////////////////
	...
	boolean sShowGrid=true, sShowFPS, sSaveTsx=false, sShowGID=false;
	...
	CheckBox cbShowGrid,cbShowFPS,cbShowGid, cbShowGidmap, cbResize; TextButton bBack3;
	...
	Table tMenu,tOpen,tNewFile,tSaveAs,tLicense;
	TextButton bNew,bOpen,bSave,bSaveAs,bExit,bBack,bLicense,bReload;
	...
	TextButton bTileMgmt,bAnimMgmt,bObjMgmt,bPreference,bProperties,bTsetMgmt,bBack2,bAutoMgmt;
	Table tRecent;
	...
	TextButton bAddTileLayer, bEditTileLayer,bReplaceTileLayer, bRemoveTileLayer, bMoveTileLayer,bBackTileLayer,bPropsTileLayer,bTerrainEditor;
...
//////////////////////////////////////////////////////
	...
	public void extractlang(){
		FileHandle handle = Gdx.files.absolute("/storage/emulated/0/AppProjects/NotTiled/app/src/main/assets/languages/Chinese");
		String text = handle.readString();
		java.lang.StringBuilder sb=new java.lang.StringBuilder();
		String filter="abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ1234567890\\{}:\"";
		for (int i=0;i<text.length();i++)
		{
			if (filter.contains(text.substring(i,i+1))) continue;
			if (sb.toString().contains(text.substring(i,i+1))) continue;
			sb.append(text.substring(i,i+1));
		}
		writeThis("Chinese.txt", sb.toString());
		
		handle = handle = Gdx.files.absolute("/storage/emulated/0/AppProjects/NotTiled/app/src/main/assets/languages/Japanese");
		...
		writeThis("Japanese.txt", sb.toString());
	...
	private void drawpicker()
	{
			...
			else
			{
				switch (tilePicker)
				{
					case "newimgobj":
						seltset=this.seltset;
					break;
					case "props":case "rnda": case "rndb":
						seltset=this.seltset;
						break;
					default:
		...
		if (tilesets.size()==0) 
		{
			...
			if (tilePicker=="terraineditor") 
			{
				uisrect(gui.newterrain);//tool switch
			}
			else if (tilePicker=="massprops") 
			{
				uisrect(gui.newterrain);
			}
			
		}
		uisrect(gui.tilesets);
		uis.end();
		ui.setProjectionMatrix(uicam.combined);
		ui.begin();
			...
			else
			{
				switch (tilePicker)
				{
					...
					case "terraineditor":
						...
						break;
					case "massprops":
						str1draw(ui, "Tileset: "+tilesets.get(selTsetID).getName(), gui.tilesets);
						str1draw(ui, "Next", gui.newterrain);
						break;
		...
		if (tilesetsize>0 && !loadingfile){
			...
			for (int jo=jon;jo <= joni;jo++)
			{
				...
				for(drawer drawer : drawers)
				{
					...
					if (sShowGIDmap) drawer.write(str1,batch);
				}
						...
						for (int i=jon;i <=joni ;i++)
						{
							if (objgroups.get(i).getObjects().size() > 0)
							{
								...
								if (ox.getShape()!=null){
									switch (ox.getShape()){
										case "text":
											str1.getData().setScale(1f+Tsw/160f);
											if(ox.getText()!=null) 
											{
												if (ox.isWrap()){
											...
											str1.getData().setScale(0.5f+Tsw/160f);
	...
	public void uisrect(gui gui)
	{
		...
		if (!landscape)
		{
			uis.rect((x+xredux)/100*ssx,(y+yredux)/100*ssy,(w-x-xredux*2)/100*ssx,(h-y-yredux*2)/100*ssy);
		}else
		{
			uis.rect((x+xredux)/100*ssy,(y+yredux)/100*ssx,(w-x-xredux*2)/100*ssy,(h-y-yredux*2)/100*ssx);
		}
	}
	...
	public void drawWorldUI(){
		...
		if (cammode=="View only"){
			if (!takingss) uisrect(gui.screenshot);//tile/obj switch
			uis.end();
			ui.setProjectionMatrix(uicam.combined);
			ui.begin();
			if (!takingss) str1draw(ui, z.screenshot,gui.screenshot);
		...
		if (roll || stamp) uisrect(gui.info);//info
			
		if (mode=="tile"||mode=="object")
		{
			uisrect(gui.mode);//tile/obj switch
			uisrect(gui.layer);//layer switch
			uisrect(gui.viewmode);//viewmode switch
			uisrect(gui.menu);//main menu button
			uisrect(gui.map);//map props. button
			uisrect(gui.tool);//tool switch
			if (autotiles.size()>0) uisrect(gui.autotile);//tool switch
		}
		
		if (mode == "tile")
		{
			uisrect(gui.rotation);//rotation switch
			uisrect(gui.undo);//undo
			uisrect(gui.redo);//redo
			uisrect(gui.picker);//select tile
			if (autotiles.size()>0) uisrect(gui.autopicker);//select auto tile
			if (showtools)
			{
				uisrect(gui.tool1);
				uisrect(gui.tool2);
		...
		if (mode=="object")
		{
			uisrect(gui.objectpicker);//objtools switch
		}
		if (mode=="newpoly")
		{
			uisrect(gui.undo);//undo
		...
		if (mode=="tile"|| mode=="object")
		{
			...
			str1draw(ui,z.menu, gui.menu);
			str1draw(ui, z.map, gui.map);
			if (autotiles.size()>0)str1draw(ui, z.autotile, gui.autotile);
		...
		if (mode == "tile")
		{
			...
			str1draw(ui, z.selecttile, gui.picker);
			if (autotiles.size()>0) str1draw(ui, z.selectautotile, gui.autopicker);
			if (stamp)
			{
				uidraw(txstamp,gui.tool,2);
			}else{
			switch (activetool)
			{
				case 0:
					uidraw(txrectangle,gui.tool,2);
					break;
				case 1:
					uidraw(txeraser,gui.tool,2);
					break;
				case 2:
					uidraw(txfill,gui.tool,2);
					break;
				case 3:
					uidraw(txcopy,gui.tool,2);
					break;
				case 4:
					if (brushsize==1){
					uidraw(txbrush,gui.tool,2);
					}else{
					uidraw(txpencil,gui.tool,2);
					}
					break;
			}
			}
			if (showtools)
			{
				uidraw(txrectangle,gui.tool1,2);
				uidraw(txeraser,gui.tool2,2);
				uidraw(txfill,gui.tool3,2);
				uidraw(txcopy,gui.tool4,2);
				if (brushsize==1){
					uidraw(txbrush,gui.tool5,2);
				}else{
					uidraw(txpencil,gui.tool5,2);
				}
			}
			str1draw(ui, viewModeName, gui.viewmode);
			
			str1draw(ui, "<<", gui.undo);
			str1draw(ui, ">>", gui.redo);
		}
		...
		else if (mode=="object")
		{
			...
			if (magnet==0)
			{
				uidraw(txunlock,gui.tool,2);
			}else{
				uidraw(txlock,gui.tool,2);
			}
...
//////////////////////////////////////////////////////
	...
	private void loadGdxStuff()
	{
		...
		ui = new SpriteBatch(); uis = new ShapeRenderer();
		sr = new ShapeRendererPlus();
		//dialog=new Dialog(z.info,skin,"");
		//str1 = new BitmapFont(); 
		
		FreeTypeFontGenerator.FreeTypeFontParameter parameter = new FreeTypeFontGenerator.FreeTypeFontParameter();
		String french="€«»ÀàÂâÆæÇçÉéÈèÊêËëÎîÏïÔôŒœÙùÛûÜüŸÿ";
		String russian = "АБВГДЕЁЖЗИЙКЛМНОПРСТУФХЦЧШЩЪЫЬЭЮЯ"
			+ "абвгдеёжзийклмнопрстуфхцчшщъыьэюя";
		String spanish = "áéíóú¿¡üñ";
		String japanese="アクティブレイヤーオジェトツル画像タセッを追加新しくする全部ニメション適用戻背景キャてコピドペスにいファ制作カムプロパ_デォリサバ接続編集楕円混合有効消ゴ終了名反転グ線高さダポラ言語管理マ送られた余白ュ上移動前開の方向設定長形去改整交換回がま付け保存と速度選択場所不透明表示地図面ソ間隔エ列数使値幅あなはモ、ボで。後発私支援者り今ぐ日々更状況得専ユザ取きう本翻訳ぱおん再起変最近-左右垂直ミズば待ちだ";
		String chinese ="活动层对象收费添加图片新所有画应用背部景取消全复制粘贴到生锈的战争创建文件自定义属性_默认目录 持续时间编辑椭圆启混合橡皮出口名翻转帧由网格高度导入夹信息关键颜色语言执照锁管理地式发送余量菜单提升称宾组好打开像向糊点多边形折线喜长方重载样品去掉改渲染订更换旋回已保存另为滚速选择位置设不透明显示在中器资源距堆支模板本瓦瓷砖柱计数类型使值能见集团宽你错误平铺邮票，来。智印章笔后！我并立即访问每日而且还服务上获得仅限赞助人户谢将变化链接第三积分最近截镜面水垂直子两者工具缩放正请稍候";
		switch (language)
		{
			case "Japanese":
				parameter.characters=FreeTypeFontGenerator.DEFAULT_CHARS + japanese;
				
				break;
			case "Chinese":
				parameter.characters=FreeTypeFontGenerator.DEFAULT_CHARS + chinese;
				
				break;
			default:
				parameter.characters=FreeTypeFontGenerator.DEFAULT_CHARS + french + russian + spanish;
		...
		if (ssx<ssy){
			if (fontsize==0) fontsize=52*ssx/1080;
			
		}else{
			if (fontsize==0) fontsize=52*ssy/1080;
		...
		if (tst.exists()){
			...
			loadtmx(extRoot+"NotTiled/"+"sample/island.tmx");
				...
				if (!fla.exists()) fla.mkdir();
					FileHandle from = Gdx.files.internal("sample/");
					from.copyTo(Gdx.files.absolute(extRoot+"NotTiled/"+"sample/"));
					prefs.putString("lof",extRoot+"NotTiled/"+"sample/island.tmx");
					prefs.putString("lastpath",extRoot+"NotTiled/");
					lastpath=extRoot+"NotTiled/sample";
					prefs.flush();
	...
	public void newtmxfileplus(boolean user){
		...
		loadtmxnewplus(extRoot+"NotTiled/sample/template/rw.tmx");
	...
	public void loadMenuMap()
	{
		...
		tMap.defaults().width(btnx).padBottom(2);
		...
		tLinks.defaults().width(btnx).height(btny).padBottom(2);
	...
	private void loadTools()
	{
		...
		flivestr=new TextField("429",skin);
		flivetset=new TextField("8",skin);
		fdeadstr=new TextField("546",skin);
		fdeadtset=new TextField("11",skin);
		...
		/*
		trandomgen.add(new Label("Smoothness",skin));
		trandomgen.add(fgencount).row();
		trandomgen.add(new Label("Var 1",skin));
		trandomgen.add(fbirthlim).row();
		trandomgen.add(new Label("Var 2",skin));
		trandomgen.add(fdeathlim).row();
		*/
		...
		/*
		trandomgen.add(new Label("Tileset 1",skin));
		trandomgen.add(flivetset).row();
		*/
		...
		/*
		trandomgen.add(new Label("Tileset 2",skin));
		trandomgen.add(fdeadtset).row();
		*/
		trandomgen.add(runrandomize).width(btnx).colspan(2);
	...
	private void beg()
	{
		...
		dialog = new Dialog(z.info,skin, "dialog")
		{
			...
			protected void result(Object object)
			{
				switch ((int) object)
				{
					case 1:
						Gdx.input.setInputProcessor(gd);
						prefs.putBoolean("donated", true).flush();
		...
		dialog.button(z.ok, 1);
		//dialog.button("Already",2);
		dialog.button(z.later,3);
		dialog.text(z.support);
		dialog.show(stage);
	...
	public void loadPropEditor(){
		...
		*/
					...
					if (sender=="mass")
					{
						for (int o=0;o<massprops.size();o++)
						{
							if (massprops.get(o))
							{
								...
								pt.getProperties().add(new property(n,t,v));
								tile tile = new tile();
								tile.setTileID(o);
								tile.setProperties(pt.getProperties());
		...
		tPropEditor.add(stack).colspan(2).left().row();
	...
	public void loadPreferences()
	{
		...
		fBgcolor.setTextFieldFilter(tffint);
		...
		*/
		...
		tPreference.add(cbEnableBlending).align(Align.left).colspan(2).row();
		tPreference.add(cbShowGrid).align(Align.left).left().colspan(2).row();
		tPreference.add(cbResize).align(Align.left).left().colspan(2).row();
		
		tPreference.add(cbShowFPS).colspan(2).align(Align.left).left().row();
		tPreference.add(cbShowGid).align(Align.left).colspan(2).left().row();
		tPreference.add(cbShowGidmap).align(Align.left).colspan(2).left().row();
	...
	public void loadPropsManagement()
	{
		...
		bProps.addListener(new ChangeListener(){
				...
				public void changed(ChangeEvent event, Actor actor)
				{
					...
					int saiz=objgroups.get(ogroup).getObjects().get(oedit).getProperties().size();
					if (saiz >= 0)
					{
						String[] srr = new String[saiz];
						for (int i=0;i < saiz;i++)
						{
							obj obb = objgroups.get(ogroup).getObjects().get(oedit);
							srr[i] = obb.getProperties().get(i).getName() + " : " + obb.getProperties().get(i).getValue();
						}
						lproplist.setItems(srr);
					}
		...
		bPropsTileLayer.addListener(new ChangeListener(){
				...
				{
					lproplist.setItems(new String[]{});
					selTileID=ltilelist.getSelectedIndex();
					int saiz=tilesets.get(selTsetID).getTiles().get(selTileID).getProperties().size();
					if (saiz >= 0)
					{
						String[] srr = new String[saiz];
						for (int i=0;i < saiz;i++)
						{
							tile obb = tilesets.get(selTsetID).getTiles().get(selTileID);
							if (obb.getProperties().size()>0){
							srr[i] = obb.getProperties().get(i).getName() + " : " + obb.getProperties().get(i).getValue();
							}
						}
						lproplist.setItems(srr);
					}
					lPropID.setText(z.tiles+" "+z.properties+": " + tilesets.get(selTsetID).getTiles().get(selTileID).getTileID());
					sender="tile";
					gotoStage(tPropsMgmt);
				}
			});
		...
		bPropertiesMap.addListener(new ChangeListener(){
				...
				{
					lproplist.setItems(new String[]{});
					int saiz=properties.size();
					if (saiz >= 0)
					{
						String[] srr = new String[saiz];
						for (int i=0;i < saiz;i++)
						{
							srr[i] = properties.get(i).getName() + " : " + properties.get(i).getValue();
						}
						lproplist.setItems(srr);
					}
		...
		bTsPropCustomProp.addListener(new ChangeListener(){
				...
				{
					lproplist.setItems(new String[]{});
					int saiz=tilesets.get(selTsetID).getProperties().size();
					if (saiz >= 0)
					{
						String[] srr = new String[saiz];
						for (int i=0;i < saiz;i++)
						{
							srr[i] = tilesets.get(selTsetID).getProperties().get(i).getName() + " : " + tilesets.get(selTsetID).getProperties().get(i).getValue();
						}
						lproplist.setItems(srr);
					}
		...
		bAutoprops.addListener(new ChangeListener(){
				...
				public void changed(ChangeEvent event, Actor actor)
				{
					...
					lproplist.setItems(new String[]{});
					int saiz=autotiles.get(dex).getProperties().size();
					if (saiz >= 0)
					{
						String[] srr = new String[saiz];
						for (int i=0;i < saiz;i++)
						{
							property obb = autotiles.get(dex).getProperties().get(i);
							srr[i] = obb.getName() + " : " + obb.getValue();
						}
						lproplist.setItems(srr);
					}
					selat=dex;
	...
	public void loadTsetManagement()
	{
		...
		bAnimMgmt = new TextButton(z.tiles, skin);
		...
		tTsetMgmt.add(bAnimMgmt).padBottom(2).row();
	...
	public void loadObjectManagement()
	{
		...
		bMoveObjLayer.addListener(new ChangeListener(){
				...
				public void changed(ChangeEvent event, Actor actor)
				{
					...
					if (dex > 0)
					{
						Collections.swap(objgroups, dex, dex - 1);
						String[] srr = new String[objgroups.size()];
						for (int i=0;i < objgroups.size();i++)
						{
							srr[i] = objgroups.get(i).getName();
	...
	public void loadNewFile(){
		...
		*/
		...
		bNNewplus= new TextButton(z.newrwmap, skin);
	...
	public void loadTileManagement()
	{
		 ...
		 */
		bAnimMgmt.addListener(new ChangeListener(){
				@Override
				public void changed(ChangeEvent event, Actor actor)
				{
		...
		bEditTileLayer = new TextButton(z.animation, skin);
		...
		tTileMgmt.add(bEditTileLayer).row();
	...
	public void loadTilesetProperties(){
		...
		*/
		...
		bTsPropChangeSource=new TextButton("Change source",skin);
		cbTsPropUseTsx=new CheckBox("Use Tsx file",skin);
		...
		bTsPropOK.addListener(new ChangeListener(){
				...
				public void changed(ChangeEvent event, Actor actor)
				{
					...
					if (fTsPropTsw.getText()!="0" && fTsPropTsw.getText()!="") t.setTilewidth(Integer.parseInt(fTsPropTsw.getText()));
					if (fTsPropTsh.getText()!="0" && fTsPropTsh.getText()!="") t.setTileheight(Integer.parseInt(fTsPropTsh.getText()));
	...
	public void loadFrameManagement()
	{
		 ...
		 */
		bEditTileLayer.addListener(new ChangeListener(){
				@Override
				public void changed(ChangeEvent event, Actor actor)
				{
					...
					if (ltilelist.getSelectedIndex() >= 0)
					{
						
						selTileID = ltilelist.getSelectedIndex();
						lFrameID.setText(z.animation+" " + tiles.get(selTileID).getTileID());
						int dex=tiles.get(selTileID).getAnimation().size();
						lframelist.setItems(new String[]{});
						String[] srr = new String[dex];
	...
	public void tujuanDialog(String dialog,FileHandle file){
		switch (dialog){
			...
			case "quickaddtset":
				...
				if (file.file().getName().toLowerCase().contains(".tsx"))
				{
					loadtsx(openedfile);
				}else
				{
...
//////////////////////////////////////////////////////
		...
		try
		{
			...
			if(properties.size()>0){
				...
				for (int m=0;m<properties.size();m++){
					...
					if (properties.get(m).getType()!=null) srz.attribute("", "type", properties.get(m).getType());
					...
					for (int i=0;i<tilesets.size();i++){
						...
						if(t.getProperties().size()>0){
							...
							for (int m=0;m<t.getProperties().size();m++){
								...
								{
									if (t.getProperties().get(m).getValue().length() <=100)
									{
				...
				switch (event)
				{
					case XmlPullParser.START_TAG:
						...
						if (name.equals("image"))
						{
							...
							String internalpath = "sample/rusted_warfare/assets/tilesets";
						...
						if (name.equals("object"))
						{
							...
							if (myParser.getAttributeValue(null, "gid")!=null) tempobj.setGid(Integer.parseInt(myParser.getAttributeValue(null, "gid")));
					...
					case XmlPullParser.END_TAG:
						...
						if (name.equals("property"))
						{
							...
							writeThis("nyet.txt",tempe.getName());
	...
	{
		tilecam.zoom=.25f;
		tilecam.position.set(ssx / 2*tilecam.zoom, -ssy / 2*tilecam.zoom + Tsh, 0);
		tilecam.update();

		cam.zoom=.25f;
		cam.position.set(ssx / 2*tilecam.zoom, -ssy / 2*tilecam.zoom + Tsh, 0);
		cam.update();
	}
...
//////////////////////////////////////////////////////
		...
		if (tapped(touch2,gui.tilesets))
		{
			if (tilesets.size()>0){
				if (kartu.equalsIgnoreCase("tile"))
				{
					...
					/*
					tilecam.position.set(SsX / 2, -SsY / 2+Tsh, 0);
					tilecam.zoom=1f;
					tilecam.update();
					*/
				}else if (kartu.equalsIgnoreCase("pickanim")){
					switch (tilePicker){
						case "newimgobj":case "props":case "rnda":case "rndb":
							if (tapped(touch2,gui.tilesetsright))
							{
								seltset+=1;
								if (seltset>=tilesets.size())
	...
	private boolean tapPick(float p1,float p2)
	{
		...
		if (kartu=="tile" || tilePicker=="newimgobj"||tilePicker=="props"||tilePicker=="rnda"||tilePicker=="rndb")
		{
			seltset=this.seltset;
		}
		else
			...
			else
			{
				...
				switch (tilePicker)
				{
					...
					case "addanim":
						...
						ltilelist.setItems(srr);
						ltilelist.setSelectedIndex(tiles.size() - 1);
						if (ltilelist.getSelectedIndex() >= 0)
						{
							selTileID = ltilelist.getSelectedIndex();
	...
	private boolean tapObject(Integer num,Vector3 touch,int ae,int ab)
	{
		try
		{
			...
			if(activeobjtool==7){
				if (copyobj==null) return true;
				nyok=(obj) copyobj.clone();
				nyok.setId(curid);
	...
	private boolean tapWorldMenu(Vector3 touch2)
	{
		...
		{
			if (mode == "tile"){
				if (tapped(touch2,gui.tool1)) {activetool=0;Gdx.input.vibrate(50);return true;}
				if (tapped(touch2,gui.tool2)) {activetool=1;Gdx.input.vibrate(50);return true;}
				if (tapped(touch2,gui.tool3)) {activetool=2;Gdx.input.vibrate(50);return true;}
				if (tapped(touch2,gui.tool4)) {activetool=3;Gdx.input.vibrate(50);return true;}
				if (tapped(touch2,gui.tool5)) {activetool=4;Gdx.input.vibrate(50);return true;}
			...
			if (mode == "tile"){
				...
				if (undolayer.size()>0){
					for (int n=undolayer.size()-1;n>=0;n--){
						layerhistory lh= undolayer.get(n);
	...
	public boolean pan(float p1, float p2, float p3, float p4)
	{
		...
		if (stamp && kartu == "tile")
		{
			...
			Integer num=-1;
			int Tha=tilesets.get(seltset).getHeight();
			int Twa=tilesets.get(seltset).getWidth();
			int Tsh=tilesets.get(seltset).getTileheight();
			int Tsw=tilesets.get(seltset).getTilewidth();
		...
		else
		{
			...
			switch (kartu)
			{
				...
				case "world":
					...
					//uicam.translate(-x, y);
					/*
					if (cam.position.x > -120*cam.zoom+(Tsw*Tw) +buffer) cam.position.x = -120*cam.zoom+(Tsw*Tw)+buffer;
					if (cam.position.y < 200*cam.zoom + Tsh -(Tsh*Th)-buffer) cam.position.y = 200*cam.zoom + Tsh -(Tsh*Th)-buffer;
					
					 if (cam.position.x < 120*cam.zoom-buffer) cam.position.x = 120*cam.zoom-buffer;
					 if (cam.position.y > -200*cam.zoom + Tsh +buffer) cam.position.y = -200*cam.zoom + Tsh+buffer;
					 */
					cam.update();
					//uicam.update();
					break;
				case "tile":case "pickanim":
					...
					if (tilecam.position.x > -120+(t.getColumns()*t.getTilewidth())+buffer) tilecam.position.x = -120+(t.getColumns()*t.getTilewidth())+buffer;
					if (tilecam.position.y < 200 + Tsh -(t.getTilecount()/t.getColumns()*t.getTileheight())-buffer) tilecam.position.y = 200 + Tsh -(t.getTilecount()/t.getColumns()*t.getTileheight())-buffer;
					
					 if (tilecam.position.x < 120-buffer) tilecam.position.x = 120-buffer;
					 if (tilecam.position.y > -200 + Tsh+buffer) tilecam.position.y = -200 + Tsh+buffer;
					*/
					tilecam.update();
			}
		}
