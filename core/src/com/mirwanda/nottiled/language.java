package com.mirwanda.nottiled;

public class language
{
	String powerof2;
	String imagesize;
	String tilesize;
	String menu;
	String map;
	String select;
	String rotate;
	String flip;
	String layer;
	String activelayer;
	String visibility;
	String tile;
	String paste;
	String object;
	String all;
	String stack;
	String single;
	String focused;
	String free;
	String lock;
	String activeobjecttool;
	String rectangle;
	String ellipse;
	String point;
	String polygon;
	String polyline;
	String text;
	String image;
	String copypaste;
	String eraser;
	String tutorial;
	String supportnottiled;
	String newfile;
	String open; 
	String save;
	String saveas;
	String discordserver;
	String whatsappgroup;
	String license;
	String reloadsamples;
	String copytorustedwarfare;
	String exporttorwmod;
	String exit;
	String back;
	String preferences;
	String background;
	String manage;
	String management;
	String filename;
	String directory;
	String selectfolder;
	String mapformat;
	String renderorder;
	String orientation;
	String createnewfile;
	String cancel;
	String opentmxfile;
	String ok;
	String info;
	String yourmaphasbeensaved;
	String selectnewlocation;
	String sampleshasbeenreloaded;
	String mapsenttorustedwatfare;
	String enableblending;
	String showgrid;
	String showfps;
	String showgidinpicker;
	String showgidinmap;
	String scrollspeed;
	String showcustomgrid;
	String gridx;
	String gridy;
	String openimagefile;
	String customproperties;
	String apply;
	String autotile;
	String tileset;
	String tilelayer;
	String objectgroup;
	String property;
	String tiles;
	String tilesetting;
	String animation;
	String terraineditor;
	String macroterraineditor;
	String macroterraininfo;
	String terrainshape;
	String teshape1x1;
	String teshape2x2;
	String teshape4x4border;
	String teshape4x4full;
	String teshape4x4corner;
	String teshape4x4cornerrev;
	String teshape6x6;
	String teshape6x6corner;
	String teshape6x6innercorner;
	String deleteterrain;
	String deleteterrainconfirm;
	String addnew;
	String rename;
	String properties;
	String moveup;
	String remove;
	String name;
	String template;
	String type;
	String value;
	String tilepicker;
	String copyall;
	String setopacity;
	String addimagetileset;
	String importcollection;
	String newcollectiontileset;
	String addtileimages;
	String appendcollectionfiles;
	String selectcollectionfiles;
	String deselectall;
	String importtsxfile;
	String importtsxfolder;
	String importfolder;
	String saveastsx;
	String set;
	String duration;
	String frame;
	String replace;
	String swap;
	String edit;
	String default_;
	String source;
	String tsxfile;
	String usetsxfile;
	String keycolor;
	String tilecolumn;
	String tilecount;
	String spacing;
	String margin;
	String id;
	String x;
	String y;
	String width;
	String height;
	String rotation;
	String language;
	String tilewidth;
	String tileheight;
	String mapwidth;
	String mapheight;
	String error;
	String file;
	String selectfile;
	String stampmode;
	String smartstampmode;
	String paintbrush;
	String later;
	String mapproperties;
	String restart;
	String links;
	String thirdpartyapps;
	String credits;
	String filesaved;
	String usetemplate;
	String loadpng;
	String recentfile;
	String screenshot;
	String screenshotsaved;
	String mirror;
	String mirrorhorizontally;
	String mirrorvertically;
	String mirrorboth;
	String tools;
	String selecttilesets;
	String selectall;
	String zoomlimit;
	String manualbook;
	String videotutorials;
	String randommap;
	String exporttopng;
	String balance;
	String picktile1;
	String picktile2;
	String randomize;
	String exportfinished;
	String resizetiles;
	String fontsize;
	String loadingfiles;
	String exporttolua;
	String exporttojson;
	String replacetiles;
	String renumbertilesetids;
	String renumbertilesetidsconfirm;
	String renumbertilesetidsok;
	String flattenobjectlayers;
	String flattenobjectlayershint;
	String flattendestination;
	String flattendeletesources;
	String flattenobjectlayersnone;
	String flattenobjectlayersnonepicked;
	String flattenobjectlayersdone;
	String objectlist;
	String objectlistsearch;
	String objectlistscopeactive;
	String objectlistscopeall;
	String objectlistbulkedit;
	String objectlistbulkset;
	String objectlistbulkreplace;
	String objectlistbulkremove;
	String objectlistbulkaddifmissing;
	String objectlistnone;
	String objectlisttruncated;
	String objectlistcount;
	String objectlistselectall;
	String objectlistselectnone;
	String objectlistdelete;
	String objectlisttypeall;
	String replacerange;
	String replacerangepick;
	String replacealllayers;
	String replacerangenone;
	String from;
	String to;
	String selecttemplate;
	String download;
	String onlinetemplate;
	String saved;
	String newnew;
	String refresh;
	String parse;
	String changesource;
	String autosaving;
	String longpress;
	String checkinternet;
	String downloadcancel;
	String downloadcomplete;
	String downloading;
	String embedtileset;
	String importtileset;
	String grid, recenter, brush, pencil, copy, fill, undo, redo;
	String brushshapesquare, brushshapecircle, brushshapediamond, brushsizeoption, floodmap, filledge;
	String export;
	String minimap,outline,clone,deleter,eraser2;
	String exportastileset;
	String duplicate;
	String move;
	String play;
	String confirmation;
	String quit;
	String yes;
	String no;
	String canceltutorial;
	String masstileproperties;
	String sendfeedback;
	String checkupdate;
	String customui;
	String circle;
	String line;
	String imagelayer;
	String collaboration;
	String remoteip;
	String opacity;
	String host;
	String join;
	String status;
	String readytoconnect;
	String connectedto;
	String listeningon;
	String failedtoconnect;
	String disconnect;
	String interval;
	String saving;
	String mapbackups;
	String prettyprinttmx;
	String backupmax;
	String backupinterval;
	String restorebackup;
	String restorebackupprompt;
	String backuprestored;
	String gridopacity;
	String additionalchars;
	String rwpath;
	String t100;
	String t101;
	String t102;
	String t103;
	String t104;
	String t200;
	String t201;
	String t202;
	String t203;
	String t204;
	String t205;
	String t206;
	String t207;
	String t208;
	String t209;
	String t210;
	String t211;
	String t212;
	String t213;
	String t214;
	String t215;
	String t216;
	String t217;
	String t218;
	String t219;
	String t220;
	String t221;
	String t222;
	String t223;
	String t300;
	String t301;
	String t302;
	String t303;
	String t304;
	String t305;
	String t306;
	String t307;
	String t308;
	String t309;
	String t310;
	String t311;
	String t312;
	String t313;
	String t314;
	String t315;
	String t316;
	String t317;
	String t318;
	String t319;
	String t320;
	String t321;
	String t322;
	String t323;
	String t324;
	String t325;
	String t326;
	String t327;
	String t328;
	String t329;
	String t330;
	String t331;
	String t332;
	String t333;
	String t334;
	String t335;
	String t336;
	String t337;
	String t338;
	String t339;
	String t340;
	String t400;
	String t401;
	String t402;
	String t403;
	String t404;
	String t405;
	String t406;
	String t407;
	String t408;
	String t409;
	String t410;
	String t411;
	String t412;
	String t413;
	String t414;
	String t415;
	String t416;
	String t417;
	String t418;
	String t419;
	String t420;
	String t421;
	String t422;
	String t423;
	String t500;
	String t501;
	String t502;
	String t503;
	String t504;
	String t505;
	String t506;
	String t507;
	String t508;
	String t509;
	String t510;
	String t511;
	String t512;
	String t513;
	String t514;
	String t515;
	String t516;
	String t517;
	String t600;
	String t601;
	String t602;
	String t603;
	String t604;
	String t605;
	String t606;
	String t607;
	String t608;
	String t609;
	String t610;
	String t611;
	String t612;
	String t700="";
	String t701="";
	String t702="";
	String t703="";
	String t704="";
	String t705="";
	String t706="";
	String t707="";
	String t708="";
	String t709="";
	String t710="";
	String charas;
	String port;
	String uieditor;
	String mirrorreverse;
	String runserver;
	String createroom;
	String joinroom;
	String clearlog;
	String pushupdate;
	String sendmessage;
	String uniqueid;
	String room;
	String netlog;
	String stopserver;
	String disconnectclient;
	String leaveroom;
	String macro;
	String runmacro;
	String macrotiles;
	String storageaccess;
	String exporttomidi;
	String dls;
	String recordwav;
	String stop;
	String recording;
	String picker;
	String showcoords;
	String exportastemplate;
	String selectionastileset;
	String nativefilechooser = "Native file chooser";
	String firstgid;
	String importx;
	String importok;
	String adjustobjects;
	String filemanager;
	String confirmdelete;
	String cut;
	String screenorientation;
	String customfont;
	String generateterrain;
	String tracebackground;
	String obj_create;
	String obj_select;
	String obj_edit;
	String obj_info;
	String multiselect;
	String removescrollboundary;
	String defaultobjectwidth;
	String defaultobjectheight;
	String newfilekindtitle;
	String newfilekindrw;
	String newfilekindmusic;
	String newfilekindpixel;
	String newfilekindempty;
	String newfilekindrwdesc;
	String newfilekindmusicdesc;
	String newfilekindpixeldesc;
	String newfilekindemptydesc;
	String moretemplates;
	String advancedoptions;
	String hideadvanced;
	String createfile;
	String aigenerate;
	String aiprompt;
	String aiapikey;
	String aiapiurl;
	String aimodel;
	String aigenerating;
	String ainokey;
	String aipaletteempty;
	String aierror;
	String aigeneratebtn;
	String aigenerated;
	String airefinehint;
	String aipixelonly;
	String aipixellayer;
	String aicanvaslarge;
	String safemodetitle;
	String safemodeprompt;
	String zip;
	String unzip;
	String showanimations;
	String grouplayer;
	String ungroup;
	String options;
	String tilecollision;
	String gid1;
	String gid2;
	String visittiledwebsite;
	String noneroot;
	String parentgroup;
	String coladdrect;
	String coladdellipse;
	String collisionshapes;
	String notilesselected;
	String nottiledlicense;
	String nottiledbasedon;
	String tilesetusedwarn;
	String areyousure;
	String setparentgroup;
	String diamond;
	String octagon;
	String plus;
	String layerlisthint;
	String editinnot2pix;
}
